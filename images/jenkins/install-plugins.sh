...
BUNDLED_DIR=/usr/share/jenkins/ref/bundled_plugins
...
doDownload() {
    ...
    bundled_jpi="$BUNDLED_DIR/${1}.jpi"
    if test -f "$bundled_jpi" && (
            unzip -p "$bundled_jpi" META-INF/MANIFEST.MF | tr -d '\r' | grep "^Plugin-Version: ${version}$" > /dev/null \
            || [[ "$version" == "latest" && -n "$NEVER_DOWNLOAD" ]]); then
        echo "Using bundled plugin: $plugin"
        cp "$bundled_jpi" "$jpi"
        return 0
    fi

    if [[ -n "$NEVER_DOWNLOAD" ]]; then
        echo "Won't download plugin: $plugin"
        return 1
    fi
