    ...
    try {
      withCredentials([file(credentialsId: 'kubeconfig', variable: 'KUBECONFIG')]) {
        withEnv([
          ...
          sh('set -o pipefail; exec 3>&1; go test -v -timeout 90m -args --exclude tweepub,tweetics --prefix j' + env.BUILD_NUMBER + '-nightly- 2>&1 3>&- | tee /dev/fd/3 | ./go-junit-report > report.xml 3>&-')
        }
      }
    } finally {
      junit('report.xml')
